...
export async function POST(
  ...
  try {
    ...
    if (!permissions?.canIn) {
      return NextResponse.json({ error: 'No permission to stock in' }, { status: 403 });
    }
    ...
    await run`
      ...
      VALUES (${id}, 'in', ${parseInt(quantity)}, ${quantityBefore}, ${quantityAfter}, ${(session.user as any).id}, ${(session.user as any).name || session.user?.name}, ${remark || null})
    `;
